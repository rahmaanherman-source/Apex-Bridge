...
import { isThemeVariant, useThemeStore } from './src/store/themeStore';
...
export default function App() {
  ...
  const initialize = useCallback(async () => {
    try {
      ...
      setTheme(isThemeVariant(settings.activeTheme) ? settings.activeTheme : 'midnight');

      // 3. Restore onboarding state
      if (settings.hasCompletedOnboarding) {
        completeOnboarding();
      }

      // 4. Determine if this is the very first launch
